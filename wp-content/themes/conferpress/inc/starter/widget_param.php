...
global $wl_options;
if((!$wl_options = get_option('conferpress')) || !is_array($wl_options) ) $wl_options = array();
...
add_action( 'sidebar_admin_setup', 'leaf_expand_control');
// adds in the admin control per widget, but also processes import/export
function leaf_expand_control(){
	...
	{	// controll-less widgets need an empty function so the callback function is called.
		if (!$wp_registered_widget_controls[$id])
			wp_register_widget_control($id,$widget['name'], 'leaf_empty_control');
		
		$wp_registered_widget_controls[$id]['callback_ct_redirect']=$wp_registered_widget_controls[$id]['callback'];
		$wp_registered_widget_controls[$id]['callback']='leaf_widget_add_custom_fields';
	...
	update_option('conferpress', $wl_options);
}

/* Empty function for callback */
function leaf_empty_control() {}

function leaf_widget_add_custom_fields() {
	...
	//var_dump(get_option('conferpress'));
	...
	echo "<p><label for='".esc_attr($id_disp)."-leafcolor'>".esc_html__('Custom Variation', 'conferpress').": <input class='widefat' type='text' name='".esc_attr($id_disp)."-leafcolor' id='".esc_attr($id_disp)."-leafcolor' value='".esc_attr($value)."' /></label></p>";
}
...
if ( is_admin() )
{
    add_action( 'sidebar_admin_setup', 'leaf_widget_style_expand_control' );
}

function leaf_widget_style_expand_control()
{   
    ...
    {   // controll-less widgets need an empty function so the callback function is called.
        if (!$wp_registered_widget_controls[$id])
            wp_register_widget_control($id,$widget['name'], 'leaf_widget_style_empty_control');
        $wp_registered_widget_controls[$id]['callback_style_redirect'] = $wp_registered_widget_controls[$id]['callback'];
        $wp_registered_widget_controls[$id]['callback'] = 'leaf_widget_style_extra_control';
        array_push( $wp_registered_widget_controls[$id]['params'], $id );   
    }
	// UPDATE CUSTOM FIELDS OPTIONS (via accessibility mode?)
	if ( isset($_POST['widget-id']) )
...
// added to widget functionality in 'leaf_widget_style_expand_control' (above)
function leaf_widget_style_empty_control() {}

// added to widget functionality in 'leaf_widget_style_expand_control' (above)
function leaf_widget_style_extra_control()
{   
    ...
    // output our extra widget logic field
    echo "
	<p id='ia-".$id_disp."'><label for='".$id_disp."-leafcolor2'>".esc_html__('Widget Style', 'conferpress').": 
 ...
 */
if(!is_admin()){
	add_filter('dynamic_sidebar_params', 'leaf_hook_before_style_widget'); 	
	function leaf_hook_before_style_widget($params){
 ...
 */
if(!is_admin()){
	add_filter('dynamic_sidebar_params', 'leaf_hook_before_width_widget'); 	
	function leaf_hook_before_width_widget($params){
		/* Add custom variation classs to widgets */
 ...
 */
if(!is_admin()){
	add_filter('dynamic_sidebar_params', 'leaf_hook_before_widget'); 	
	function leaf_hook_before_widget($params){
		/* Add custom variation classs to widgets */
		global $wl_options;
		$id=$params[0]['widget_id'];
