<?php
class leaf_walker_nav_menu extends Walker_Nav_Menu {
	...
	// add main/sub classes to li's and links
	 function start_el( &$output, $item, $depth = 0, $args = array(),  $current_object_id = 0) {
		...
		$item_output = sprintf( '%1$s<a%2$s'.($is_parent&&$depth==0?' data-toggle="dropdown"':'').'>'.$leaf_icon_names.'%3$s%4$s%5$s ' . ($item->description&&$depth==0?'<span class="menu-description">'.$item->description.'</span>':'') .'</a>%6$s',
			...
			apply_filters( 'the_title',(empty($item->title) ? $item->post_title : $item->title ) , $item->ID ),
...
class leaf_walker_nav_menu_mobile extends Walker_Nav_Menu {
	...
	// add main/sub classes to li's and links
	 function start_el( &$output, $item, $depth = 0, $args = array(), $current_object_id = 0) {
		...
		$item_output = sprintf( '%1$s%3$s%4$s%5$s%6$s',
			...
			$attributes,
			is_array($args) ? $args['link_before'] : $args->link_before,
			apply_filters( 'the_title', $sub_menu . (empty($item->title) ? $item->post_title : $item->title ) , $item->ID ),
			is_array($args) ? $args['link_after']  : $args->link_after,
			is_array($args) ? $args['after'] : $args->after
		);
...
add_filter( 'wp_nav_menu_objects', 'leaf_add_menu_parent_class' );
function leaf_add_menu_parent_class( $items ) {
